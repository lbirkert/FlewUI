...
export const POST: RequestHandler = async ({ request, locals, params }) => {
  const ctx = await getDriveContext(params.driveId, locals);
  if (!ctx) return json({ error: "Drive not found" }, { status: 404 });
  if (ctx.type === "share") return json({ error: "Create folder not available in shared drives" }, { status: 403 });
  ...
  const folder = await createFolder(name.trim(), ctx.userId, parentId);
